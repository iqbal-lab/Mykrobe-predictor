...
int parse_cmdline_inner_loop(int argc, char* argv[], int unit_size, CmdLine* cmdline_ptr, char* error_string)
{
  ...
  while ((opt) > 0) {
    ...
    case 'h':
      {
	printf("***********************\n");
	printf("Mykrobe.predictor for {{selfer.species_long_name}}\n");
	printf("***********************\n");
    ...
    case 'z':
      {
	printf("Mykrobe predictor for {{selfer.species_long_name}} Version %d.%d.%d\n", MYK_VERSION,MYK_SUBVERSION,MYK_SUBSUBVERSION);
	exit(0);
	break;
      }      
	...
	strbuf_append_str(tmp, "data/{{selfer.species}}");
