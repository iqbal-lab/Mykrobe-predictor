...
int main(int argc, char **argv)
{
  ...
  if (cmd_line->format==Stdout){
    printf("myKrobe.predictor for {{selfer.species}}, version %d.%d.%d.%d"VERSION_STR"\n",
  ...
  if (cmd_line->method==WGAssemblyThenGenotyping)
  ...
  else if (cmd_line->method==InSilicoOligos)
    {
      ...
      strbuf_append_str(sk, "data/skeleton_binary/{{selfer.species}}/skeleton.k15.ctx");
      if ( (access(sk->buff,F_OK)!=0) || (WINDOWS==1) )
  {
    ...
    strbuf_append_str(skeleton_flist, 
          "data/skeleton_binary/{{selfer.species}}/list_speciesbranches_genes_and_muts");
    uint64_t dummy=0;
    boolean is_rem=true;
    //timestamp();
  ...
  if (cmd_line->min_frac_to_detect_minor_pops == 0.0){
    if (err_rate<0.02)
    {
      cmd_line->min_frac_to_detect_minor_pops = 0.5;
    }
  else
    {
      cmd_line->min_frac_to_detect_minor_pops = 0.5;
