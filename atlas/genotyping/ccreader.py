...
class ColourCovgsRead(object):
    ...
    def __str__(self):
        return "%s: %i coverage on %i%% of seq" % (self.name,self.median_non_zero_coverage, self.percent_non_zero_coverage )
    ...
    def percent_non_zero_coverage(self):
        return int(100 * float(sum([i > 0 for i in self.covgs])) / len(self.covgs))
