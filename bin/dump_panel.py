...
def get_context(pos):
	context = []
	for variant in Variant.objects(start__ne = pos, start__gt = pos - args.kmer, start__lt = pos + args.kmer):
...
def seen_together(variants):
	...
	samples_seen_more_than_once = [k for k,v in samples_counter.iteritems() if v > 1] ## 2 as we also have global var
	contexts = []
...
for variant in Variant.snps():
	...
	sys.stdout.write(">ref-%s?num_alts=%i&ref=%s\n" % (variant_panel.variant.var_hash, len(variant_panel.alts), variant_panel.variant.reference.id))
	sys.stdout.write("%s\n" % variant_panel.ref)
