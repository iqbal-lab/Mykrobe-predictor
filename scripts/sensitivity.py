...
parser = argparse.ArgumentParser(
    description='Genotype a sample based on kmer coverage on alleles')
...
try:
    ...
    for atlas_gt_call_set in atlas_gt_call_sets:
        ...
        # print "CallSet", "union", "vars_in_callset", "intersection",
        # "missing", "extra", "intersection/union"
        ...
        for variant_set in variant_sets:
            cur_variant_set = set(
                Variant.objects(
                    variant_set=variant_set).distinct('name_hash'))
        ...
        if args.diff:
            ...
            for nh in genotyped_set - discover_variant_set:
                ...
                # v = vp.variant
                # print v.name
                gv = TypedVariant.objects.get(
                    name_hash=nh, call_set=atlas_gt_call_set)
            ...
            for nh in discover_variant_set - genotyped_set:
                vs = Variant.objects(
                    variant_set__in=variant_sets, name_hash=nh)
                for v in vs:
